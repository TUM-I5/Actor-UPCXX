...
class PingPongActor : public ActorImpl
{
  ...
  private:
    InPort<std::vector<size_t>, 10> *ip;
    OutPort<std::vector<size_t>, 10> *op;
    bool begin;
    PingPongActor(
        // std::thread actorThread, -> to be reconstructed
        // upcxx::persona actorPersona, //get it from new rank
        bool isRunning,
  ...
  public:
    ...
    PingPongActor(std::string &&name, int o, int s, int t);
    PingPongActor(PingPongActor &&other);
    PingPongActor(ActorData &&ad);
    PingPongActor(const PingPongActor &other) = delete;
    ~PingPongActor();
    PingPongActor(std::string &&name, std::variant<std::monostate> &&emtpyvar);
    PingPongActor(const std::string &name, const std::variant<std::monostate> &emtpyvar);
