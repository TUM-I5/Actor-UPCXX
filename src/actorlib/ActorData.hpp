...
class ActorData
{
  public:
    ...
    ActorData(std::string &&name, size_t workTokens, size_t workTime, upcxx::intrank_t &&mark, ActorState state,
              std::array<unsigned int, history_array_length> &&historyArray, size_t index, bool initial_act, bool term)
        : name(std::move(name)), workTokens(workTokens), workTime(workTime), mark(std::move(mark)), state(state),
          actor_cost(std::move(historyArray)), index(index), initial_act(initial_act), term(term)
    {
    ...
    ActorData(const std::string &name, size_t workTokens, size_t workTime, const upcxx::intrank_t &mark,
              ActorState state, const std::array<unsigned int, history_array_length> &historyArray, size_t index,
              bool initial_act, bool term)
        : name(name), workTokens(workTokens), workTime(workTime), mark(mark), state(state), actor_cost(historyArray),
          index(index), initial_act(initial_act), term(term)
    {
        if (state != ActorState::TemporaryStoppedForMigration && state != ActorState::Terminated)
        {
            throw std::runtime_error("Constructing an Actor with an illegal state for init");
    ...
    ActorData(const ActorData &ad)
        : name(ad.name), workTokens(ad.workTokens), workTime(ad.workTime), mark(ad.mark), state(ad.state),
          actor_cost(ad.actor_cost), index(ad.index), initial_act(ad.initial_act), term(ad.term)
    {
        if (state != ActorState::TemporaryStoppedForMigration && state != ActorState::Terminated)
        {
            throw std::runtime_error("Constructing an Actor with an illegal state for init");
    ...
    struct upcxx_serialization
    {
        ...
        template <typename Reader> static ActorData *deserialize(Reader &reader, void *storage)
        {
            ...
            size_t workTokens = reader.template read<size_t>();
            size_t workTime = reader.template read<size_t>();
