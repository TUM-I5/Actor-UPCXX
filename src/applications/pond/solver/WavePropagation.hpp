...
#define uRight (u[1])
    ...
    T bLeft;
    //! bathymetry on the right side of the edge (could change during
    //! execution).
    T bRight;

    //! velocity on the left side of the edge (computed by
    //! determineWetDryState).
    T uLeft;
    //! velocity on the right side of the edge (computed by
    //! determineWetDryState).
    T uRight;
#endif
    ...
    enum WetDryState
    {
        ...
        WetDryInundation,     /**< 1st cell: wet, 2nd cell: dry. 1st cell lies
                                 higher than the 2nd one. */
        WetDryWall,           /**< 1st cell: wet, 2nd cell: dry. 1st cell lies lower than
                               * the 2nd one.
                               *   Momentum is not large enough to overcome the
                               * difference. */
        WetDryWallInundation, /**< 1st cell: wet, 2nd cell: dry. 1st cell lies
                               * lower than the 2nd one. Momentum is large
                               * enough to overcome the difference. */
        DryWetInundation,     /**< 1st cell: dry, 2nd cell: wet. 1st cell lies lower
                                 than the 2nd one. */
        DryWetWall,           /**< 1st cell: dry, 2nd cell: wet. 1st cell lies higher than
                               * the 2nd one.
                               *   Momentum is not large enough to overcome the
                               * difference. */
        DryWetWallInundation  /**< 1st cell: dry, 2nd cell: wet. 1st cell lies
                               * higher than the 2nd one. Momentum is large
                               * enough to overcome the difference. */
    };

    //! wet/dry state of our Riemann-problem (determined by
    //! determineWetDryState)
    ...
    WavePropagation(T i_dryTolerance, T i_gravity, T i_zeroTolerance)
        : dryTol(i_dryTolerance), g(i_gravity), zeroTol(i_zeroTolerance){};
     ...
     */
    inline void storeParameters(const T &i_hLeft, const T &i_hRight, const T &i_huLeft, const T &i_huRight,
  ...
  public:
     ...
     * This is the default method every standalone wave propagation solver
     * should provide.
     *
     * @param i_hLeft height on the left side of the edge.
     * @param i_hRight height on the right side of the edge.
     ...
     */
    virtual void computeNetUpdates(const T &i_hLeft, const T &i_hRight, const T &i_huLeft, const T &i_huRight,
                                   const T &i_bLeft, const T &i_bRight,
     ...
     */
    void setDryTolerance(const T i_dryTolerance) { dryTol = i_dryTolerance; }
