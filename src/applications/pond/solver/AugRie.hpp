...
template <typename T> class solver::AugRie : public WavePropagation<T>
{
  private: // explicit for unit tests
    ...
    {
        DrySingleRarefaction,  /**< 1st wave family: contact discontinuity; 2nd
                                  wave family: rarefaction. */
        SingleRarefactionDry,  /**< 1st wave family: rarefaction; 2nd wave
                                  family: contact discontinuity. */
        ShockShock,            /**< 1st wave family: shock; 2nd wave family: shock. */
        ShockRarefaction,      /**< 1st wave family: shock; 2nd wave family:
                                  rarefaction. */
        RarefactionShock,      /**< 1st wave family: rarefaction; 2nd wave family:
                                  shock. */
        RarefactionRarefaction /**< 1st wave family: rarefaction; 2nd wave
                                  family: rarefaction. */
    };
  ...
  public:
     ...
     */
    AugRie(T i_dryTolerance = (T)0.01, T i_gravity = (T)9.81, T i_newtonTolerance = (T)0.000001,
           int i_maxNumberOfNewtonIterations = 10, T i_zeroTolerance = (T)0.00001)
        : WavePropagation<T>(i_dryTolerance, i_gravity, i_zeroTolerance), newtonTol(i_newtonTolerance),
...
#define sqrt_hRight (sqrt_h[1])
     ...
     */
    void computeNetUpdates(const T &i_hLeft, const T &i_hRight, const T &i_huLeft, const T &i_huRight, const T &i_bLeft,
                           const T &i_bRight,

                           T &o_hUpdateLeft, T &o_hUpdateRight, T &o_huUpdateLeft, T &o_huUpdateRight, T &o_maxWaveSpeed
  ...
  private:
     ...
     * (eigenvalues).
     */
    inline void computeWaveDecomposition(T o_fWaves[3][2], T o_waveSpeeds[3]
#if CONFIG_TSUNAMI_AUGMENTED_RIEMANN_EIGEN_COEFFICIENTS
                                         ,
                                         T o_eigenCoefficients[3]
#endif
    )
    {
        ...
        if ((riemannStructure == ShockRarefaction || riemannStructure == RarefactionShock ||
            ...
            hMiddle > dryTol)
        { // limit to ensure non-negative depth

            // TODO: GeoClaw, riemann_aug_JCP; No explicit boundaries for
            // "strong rarefaction" in literature?
            T rareBarrier[2] = {.5, .9};
            ...
            if ((riemannStructure == RarefactionShock || riemannStructure == RarefactionRarefaction) &&
                eigenValues[0] * middleStateSpeeds[0] < (T).0)
            { // transsonic rarefaction, first wave family
                rareBarrier[0] = (T).2;
            }
            else if ((riemannStructure == ShockRarefaction || riemannStructure == RarefactionRarefaction) &&
                     eigenValues[2] * middleStateSpeeds[1] < (T)0.)
            { // transsonic rarefaction, second wave
              // family
                rareBarrier[0] = (T).2;
            }

            T sqrt_g_hMiddle = std::sqrt(g * hMiddle);
            // determine the max. rarefaction size (distance between head and
            // tail)
            T rareFactionSize[2];
            ...
            // set the eigenvalue of the corrector wave in the case of a "strong
            // rarefaction"
            ...
            //        if ( hMiddle < std::min(hLeft, hRight)/5.) { //middle
            //        state in an HLL solve
        ...
        rightHandSide[2] =
            huRight * uRight + (T).5 * g * hRight * hRight - (huLeft * uLeft + (T).5 * g * hLeft * hLeft);
        ...
        if ((std::fabs(lLambdaBar) < zeroTol) || (lLambdaBar * lLambdaTilde < zeroTol) ||
     ...
     */
    inline void computeMiddleState(const T &i_hLeft, const T &i_hRight, const T &i_uLeft, const T &i_uRight,
                                   const T &i_huLeft, const T &i_huRight, const int &i_maxNumberOfNewtonIterations = 1)
    {
        ...
        if (riemannStructure == ShockShock)
        {
             ...
             *    u_l - (h_m - h_l) \sqrt{\frac{g}{2} \left( \frac{1}{h_m} +
             *\frac{1}{h_l} \right) } =  u_r + (h_m - h_r) \sqrt{\frac{g}{2}
             *\left( \frac{1}{h_m} + \frac{1}{h_r} \right) }
             *
             ...
             *\begin{equation}
             *  \begin{matrix}
             *    \phi_{ss}'(h) = &&\sqrt{\frac{g}{2} \left( \frac{1}{h} +
             *\frac{1}{h_l} \right) }
             *                      +\sqrt{\frac{g}{2} \left( \frac{1}{h} +
             *\frac{1}{h_r} \right) }\\
             *                    && -\frac{g}{4}
             *                        \frac{h-h_l}
             *                        {
             *                         h^2\sqrt{\frac{g}{2} \left( \frac{1}{h} +
             *\frac{1}{h_l} \right) }
             *                        }-
             *                        \frac{g}{4}
             *                        \frac{h-h_r}
            ...
            {
                //          sqrtTermHLow = std::sqrt((T).5 * g * ((T)1/hMiddle +
                //          (T)1/hLeft));
                l_sqrtTermH[0] = std::sqrt((T).5 * g * ((hMiddle + i_hLeft) / (hMiddle * i_hLeft)));
                //          sqrtTermHHigh = std::sqrt((T).5 * g * ((T)1/hMiddle
                //          + (T)1/hRight));
                ...
                //            std::endl; std::cerr << "std::fabs(phi): " <<
                //            std::fabs(phi) << std::endl; std::cerr <<
                //            "hMiddle: " << hMiddle << std::endl;
                //            assert(false);
                //          }
            }
            ...
            //  u_m = u_l - (h_m - h_l) \sqrt{\frac{g}{2} \left( \frac{1}{h_m} +
            //  \frac{1}{h_l} \right) }
            //\end{equation}
            //
            //\begin{equation}
            //  \label{eq:hmshock2ndfamsimp}
            //  u_m = u_r + (h_m - h_r) \sqrt{\frac{g}{2} \left( \frac{1}{h_m} +
            //  \frac{1}{h_r} \right) }
            //\end{equation}

            //        T uMiddleEstimates[2];
            //        uMiddleEstimates[0] = uLeft - (hMiddle - hLeft) *
            //        l_sqrtTermH[0]; uMiddleEstimates[1] = i_uRight + (hMiddle
            //        - hRight) * l_sqrtTermH[1]; uMiddle = (T).5 *
            //        (uMiddleEstimates[0] + uMiddleEstimates[1]);

            // middle state speeds as they are implemented in clawpack, TODO:
            // why?
            //        middleStateSpeeds[0] = uMiddleEstimates[0] -
            //        sqrt_g_hMiddle; middleStateSpeeds[1] = uMiddleEstimates[1]
            //        + sqrt_g_hMiddle;
        }
        ...
        if (riemannStructure == RarefactionRarefaction)
        {
            ...
            hMiddle =
                std::max((T)0.,
                         i_uLeft - i_uRight + (T)2. * (l_sqrt_g_hLeft + l_sqrt_g_hRight)); // std::max -> Text after
                                                                                           // formula (13.56), page 279
            hMiddle = (T)1 / ((T)16 * g) * hMiddle * hMiddle;

            sqrt_g_hMiddle = std::sqrt(g * hMiddle);

            // middle state speeds as they are implemented in clawpack, why?
            //        middleStateSpeeds[0] = uLeft + (T)2.*l_sqrt_g_hLow -
            //        (T)3.*sqrt_g_hMiddle; middleStateSpeeds[1] = i_uRight -
            //        (T)2.*l_sqrt_g_hHigh + (T)3.*sqrt_g_hMiddle;
        }
        ...
        if (riemannStructure == ShockRarefaction || riemannStructure == RarefactionShock)
        {
            ...
            for (int i = 0; i < i_maxNumberOfNewtonIterations; i++)
            {
                 ...
                 *\begin{equation}
                 *  \begin{matrix}
                 *    \phi_{sr}(h) = \phi_{rs}(h) =&& u_r - u_l + (h - h_{min})
                 *\sqrt{ \frac{g}{2} \left( \frac{1}{h} + \frac{1}{h_{min}}
                 *\right)
                 *                                          } \\
                 ...
                 *\end{equation}
                 *\begin{equation}
                 *    \phi'_{sr}(h) = \phi'_{rs}(h) = \sqrt{\frac{g}{2} \left(
                 *\frac{1}{h} + \frac{1}{h_{min}} \right)  }
            ...
            //        //middle state speeds as they are implemented in clawpack,
            //        TODO: why? if (riemannStructure == ShockRarefaction) {
            //          uMiddle = i_uRight - (T)2.*( l_sqrt_g_hHigh -
            //          sqrt_g_hMiddle ); middleStateSpeeds[0] = i_uRight -
            //          (T)2.*l_sqrt_g_hHigh + sqrt_g_hMiddle;
            //          middleStateSpeeds[1] = i_uRight - (T)2.*l_sqrt_g_hHigh +
            //          (T)3.*sqrt_g_hMiddle;
            //        }
            //        else {
            //          uMiddle = uLeft + (T)2.*( l_sqrt_g_hLow - sqrt_g_hMiddle
            //          ); middleStateSpeeds[0] = uLeft + (T)2.*l_sqrt_g_hLow -
            //          (T)3.*sqrt_g_hMiddle; middleStateSpeeds[1] = uLeft +
            //          (T)2.*l_sqrt_g_hLow - sqrt_g_hMiddle;
            //        }
        }
     ...
     */
    inline RiemannStructure determineRiemannStructure(const T &i_hLeft, const T &i_hRight, const T &i_uLeft,
                                                      const T &i_uRight) const
    {
    ...
    static inline void solveLinearEquation(const T i_matrix[3][3], const T i_b[3], T o_x[3])
    {
        ...
        const T m[3][3] = {{(i_matrix[1][1] * i_matrix[2][2] - i_matrix[1][2] * i_matrix[2][1]),
        ...
        //      }
        //      else {
        //        std::cerr << "Warning: Linear dependent eigenvectors! (using
        //        Jacobi Solver)" << std::endl; std::cerr << "Determinant: " <<
        //        origDet << std::endl; std::cerr << i_matrix[0][0] << "\t" <<
        //        i_matrix[0][1] << "\t" << i_matrix[0][2] << std::endl;
        //        std::cerr << i_matrix[1][0] << "\t" << i_matrix[1][1] << "\t"
        //        << i_matrix[1][2] << std::endl; std::cerr << i_matrix[2][0] <<
        //        "\t" << i_matrix[2][1] << "\t" << i_matrix[2][2] << std::endl;
        //#if 0
        //        T xTemp[3];
        ...
        //          }
        //
        //          if (fabs(o_x[0]-xTemp[0]) + fabs(o_x[1]-xTemp[1]) +
        //          fabs(o_x[2]-xTemp[2]) < zeroTol*10.)
        //            break;
        //          else {
        //            std::cout << "Error: " << fabs(o_x[0]-xTemp[0]) +
        //            fabs(o_x[1]-xTemp[1]) + fabs(o_x[2]-xTemp[2]) <<
        //            std::endl; xTemp[0] = o_x[0]; xTemp[1] = o_x[1]; xTemp[2]
        //            = o_x[2];
        //          }
        //        }
    ...
    //     * using formula: |A|=a11 * (a22a33-a32a23) + a12 * (a23a31-a33a21) +
    //     a13 * (a21a32-a31a22)
    //     *                |A|=a00 * (a11a22-a21a12) + a01 * (a12a20-a22a10) +
    //     a02 * (a10a21-a20a11)
    //     */
    //    T computeDeterminant(T matrix[3][3]) const
    //    {
    //
    //      T determinant =     matrix[0][0] * ( matrix[1][1] * matrix[2][2] -
    //      matrix[2][1] * matrix[1][2] )
    //                        		    + matrix[0][1] * ( matrix[1][2] *
    //                        matrix[2][0] - matrix[2][2] * matrix[1][0] )
