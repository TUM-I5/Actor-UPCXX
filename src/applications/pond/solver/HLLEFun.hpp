 ...
 *
 */
template <typename T> class HLLEFun
{
  private:
    const T dryTol;
  ...
  public:
     ...
     * wall boundary conditions are applied (default value is 0.01)
    ...
    HLLEFun(T i_dryTol = (T)0.01, T i_gravity = (T)9.81, T i_zeroTol = (T)0.0000001)
        : dryTol(i_dryTol), g(i_gravity), half_g((T).5 * i_gravity), sqrt_g(std::sqrt(i_gravity)), zeroTol(i_zeroTol)
    {
    }

// Define MIN and MAX as Macros, std::min and std::max seem to crash icpc
// For some reason, the Intel C++ compiler crashes during when Vectorization is
// enabled.
...
#pragma omp declare simd
    ...
    void computeNetUpdates(const T i_hLeft, const T i_hRight, const T i_huLeft, const T i_huRight, const T i_bLeft,
                           ...
                           T &o_hUpdateLeft, T &o_hUpdateRight, T &o_huUpdateLeft, T &o_huUpdateRight,
            ...
            { // hLeft >= dryTol and hRight < dryTol
                ...
                { // hLeft >= dryTol and hRight < dryTol and hLeft +
                  // bLeft <= bRight
                    ...
                    // Removed middle state computation -> assuming momentum is
                    // never high enough to overcome boundary on its own
                    hRight = hLeft;
                    uRight = -uLeft;
                    huRight = -huLeft;
                    bRight = bLeft = (T)0;
...
// nothing to do for dry/dry case, all netUpdates and maxWaveSpeed are 0
#ifdef VECTORIZE // Vectorization fails with branching return statement. Let
                 // computation resume with dummy values instead.
                hLeft = hRight = dryTol;
                uLeft = uRight = (T)0;
                huLeft = huRight = (T)0;
                bLeft = bRight = (T)0;
        ...
        {
// This case corresponds to the early return statement above. Should only be
// executed with vectorization enabled.
#ifdef VECTORIZE
        ...
        const T hLLMiddleHeight =
            MAX((huLeft - huRight + extEinfeldtSpeeds[1] * hRight - extEinfeldtSpeeds[0] * hLeft) /
                    (extEinfeldtSpeeds[1] - extEinfeldtSpeeds[0]),
                (T)0);

        // define eigenvalues
        const T eigenValues[3] = {extEinfeldtSpeeds[0], (T)0.5 * (extEinfeldtSpeeds[0] + extEinfeldtSpeeds[1]),
                                  extEinfeldtSpeeds[1]};

        // define eigenvectors
        // MB: no longer used as system matrix
        //     -> but still used to compute f-waves
        const T eigenVectors[3][3] = {{(T)1, (T)0, (T)1},
                                      {eigenValues[0], (T)0, eigenValues[2]},
                                      {eigenValues[0] * eigenValues[0], (T)1, eigenValues[2] * eigenValues[2]}};

        // compute the jump in state
        T rightHandSide[3] = {hRight - hLeft, huRight - huLeft,
        ...
        if (eigenValues[0] < -zeroTol && eigenValues[2] > zeroTol)
        {
            // subsonic
            steadyStateWave[0] =
                MAX(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[0]);
            steadyStateWave[0] =
                MIN(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[2]);
        }
        else if (eigenValues[0] > zeroTol)
        {
            ...
            steadyStateWave[0] =
                MIN(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[0]);
        }
        else if (eigenValues[2] < -zeroTol)
        {
            // supersonic left TODO: motivation?
            steadyStateWave[0] =
                MAX(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[2]);
        ...
        const T m[3][3] = {{(eigenVectors[1][1] * eigenVectors[2][2] - eigenVectors[1][2] * eigenVectors[2][1]),
        ...
        const T beta[3] = {(m[0][0] * rightHandSide[0] + m[0][1] * rightHandSide[1] + m[0][2] * rightHandSide[2]) * s,
        ...
        T fWaves[3][2];  // array to store the three f-wave vectors (2 components
                         // each)
        ...
        else
        {
            ...
            for (int waveNumber = 0; waveNumber < 3; waveNumber++) {
                    fWaves[waveNumber][0] = beta[waveNumber] *
            eigenVectors[1][waveNumber]; //select 2nd and fWaves[waveNumber][1]
            = beta[waveNumber] * eigenVectors[2][waveNumber]; //3rd component of
            the augmented decomposition
            }
            */
...
#ifdef VECTORIZE
        // DryDry state with dummy data only reaches this point with
        // vectorization enabled. Return values are not updated in that case,
        // since nothing happens in those cells.
        if (wetDryState != DryDry)
        {
            ...
            for (int waveNumber = 0; waveNumber < 3; waveNumber++) {
                    if (waveSpeeds[waveNumber] < -zeroTol) {
                            ...
                            //TODO: this case should not happen mathematically,
            but it does. Where is the bug? Machine accuracy only?
                            // MB: >>> waveSpeeds set to 0 for cases WetDryWall
            and DryWetWall o_hUpdateLeft  += (T)0.5 * fWaves[waveNumber][0];
