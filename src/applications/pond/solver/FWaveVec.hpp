 ...
 *
 ****
 **** This is a vectorizable C++ implementation of the F-Wave solver
 *(FWave.hpp).
 ****
 *
 ...
 *    Homepage:
 *http://www5.in.tum.de/wiki/index.php/Sebastian_Rettenberger,_M.Sc. E-Mail:
 *rettenbs AT in.tum.de Some optimzations: Michael Bader Homepage:
 *http://www5.in.tum.de/wiki/index.php/Michael_Bader E-Mail: bader AT in.tum.de
 *
 ****
 *
 * (Main) Literature:
 ...
 *
 */
template <typename T> class FWaveVec
{
  private:
    const T dryTol;
  ...
  public:
    ...
    /**
     * takes the water height, discharge and bathymatry in the left and right
     * cell and computes net updates (left and right going waves) according to
     * the f-wave approach. It also returns the maximum wave speed.
     */
    void computeNetUpdates(T i_hLeft, T i_hRight, T i_huLeft, T i_huRight, T i_bLeft, T i_bRight,
                           ...
                           T &o_maxWaveSpeed) const
    {
        ...
        fWaveComputeWaveSpeeds(i_hLeft, i_hRight, i_huLeft, i_huRight, uLeft, uRight, i_bLeft, i_bRight,

                               waveSpeeds0,
                               waveSpeeds1); // 20 FLOPs (incl. 3 sqrt, 1 div, 2 min/max)

        //! variables to store the two f-waves
        T fWaves0 = 0., fWaves1 = 0.;

        // compute the decomposition into f-waves
        fWaveComputeWaveDecomposition(i_hLeft, i_hRight, i_huLeft, i_huRight, uLeft, uRight, i_bLeft, i_bRight,

                                      waveSpeeds0, waveSpeeds1, fWaves0,
                                      fWaves1); // 23 FLOPs (incl. 1 div)
    ...
    inline void fWaveComputeWaveSpeeds(const T i_hLeft, const T i_hRight, const T i_huLeft, const T i_huRight,
                                       const T i_uLeft, const T i_uRight, const T i_bLeft, const T i_bRight,
                                       ...
                                       T &o_waveSpeed0, T &o_waveSpeed1) const
    {
        ...
        o_waveSpeed1 = std::max(characteristicSpeed1,
                                roeSpeed1); // 1 FLOP (max)
                                            //==============
                                            // 20 FLOPs (incl. 3 sqrt, 1 div, 2 min/max)
    }

    inline void fWaveComputeWaveDecomposition(const T i_hLeft, const T i_hRight, const T i_huLeft, const T i_huRight,
                                              const T i_uLeft, const T i_uRight, const T i_bLeft, const T i_bRight,
