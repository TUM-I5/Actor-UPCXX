 ...
 *            title={A wave propagation method for conservation laws and balance
 *laws with spatially varying flux functions}, author={Bale, D.S. and LeVeque,
 *R.J. and Mitran, S. and Rossmanith, J.A.}, journal={SIAM Journal on Scientific
 *Computing}, volume={24}, number={3}, pages={955--978}, year={2002},
 *            publisher={Citeseer}}
 *
...
template <typename T> class solver::FWave : public WavePropagation<T>
{
  ...
  public:
     ...
     *
     * Please note:
     *   In the case of a Dry/Wet- or Wet/Dry-boundary, wall boundary conditions
     * will be set. The f-Wave solver is not positivity preserving.
     *   -> You as the programmer have to take care about "negative water
     * heights"!
     *
     ...
     *
     * @param o_hUpdateLeft will be set to: Net-update for the height of the
     * cell on the left side of the edge.
     * @param o_hUpdateRight will be set to: Net-update for the height of the
     * cell on the right side of the edge.
     * @param o_huUpdateLeft will be set to: Net-update for the momentum of the
     * cell on the left side of the edge.
     * @param o_huUpdateRight will be set to: Net-update for the momentum of the
     * cell on the right side of the edge.
     * @param o_maxWaveSpeed will be set to: Maximum (linearized) wave speed ->
     * Should be used in the CFL-condition.
     */
                           ...
                           T &o_maxWaveSpeed)
    {
        ...
        computeNetUpdatesWithWaveSpeeds(waveSpeeds, o_hUpdateLeft, o_hUpdateRight, o_huUpdateLeft, o_huUpdateRight,
                                        o_maxWaveSpeed);
    ...
    /**
     * Compute net updates for the cell on the left/right side of the edge.
     * This is an expert method, because a lot of (numerical-)knowledge about
     * the problem is assumed/has to be provided. It is the f-Wave entry point
     * for the hybrid solver,  which combines the "simple" F-Wave approach with
     * the more complex Augmented Riemann Solver.
     *
     ...
     * Should be used in the CFL-condition.
     */
    void computeNetUpdatesHybrid(const T &i_hLeft, const T &i_hRight, const T &i_huLeft, const T &i_huRight,
                                 const T &i_bLeft, const T &i_bRight, const T &i_uLeft, const T &i_uRight,
    ...
    {
        // store parameters to member variables
        storeParameters(i_hLeft, i_hRight, i_huLeft, i_huRight, i_bLeft, i_bRight, i_uLeft, i_uRight);

        computeNetUpdatesWithWaveSpeeds(i_waveSpeeds, o_hUpdateLeft, o_hUpdateRight, o_huUpdateLeft, o_huUpdateRight,
                                        o_maxWaveSpeed);
    }
  ...
  private:
     ...
     * Should be used in the CFL-condition.
     */
    void computeNetUpdatesWithWaveSpeeds(const T i_waveSpeeds[2],
                                         ...
                                         T &o_maxWaveSpeed)
    {
        ...
        // 1st wave family
        if (i_waveSpeeds[0] < -zeroTol)
        { // left going
            o_hUpdateLeft += fWaves[0][0];
            o_huUpdateLeft += fWaves[0][1];
        }
        else if (i_waveSpeeds[0] > zeroTol)
        { // right going
        ...
        // 2nd wave family
        if (i_waveSpeeds[1] < -zeroTol)
        { // left going
            o_hUpdateLeft += fWaves[1][0];
            o_huUpdateLeft += fWaves[1][1];
        }
        else if (i_waveSpeeds[1] > zeroTol)
        { // right going
     ...
     */
    void computeWaveDecomposition(const T i_waveSpeeds[2], T o_fWaves[2][2]) const
    {
        ...
        // assert: wave speed of the 1st wave family should be less than the
        // speed of the 2nd wave family.
        ...
        fDif[1] = huRight * uRight + (T).5 * g * hRight * hRight - (huLeft * uLeft + (T).5 * g * hLeft * hLeft);
