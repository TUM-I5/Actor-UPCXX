...
__device__ inline void computeMiddleState(const real &i_hLeft, const real &i_hRight, const real &i_uLeft,
                                          const real &i_uRight, const real dryTol, const real newtonTol, const real g,
                                          const real sqrt_g, const unsigned int &i_maxNumberOfNewtonIterations,
                                          real &o_hMiddle, real o_middleStateSpeeds[2]);
 ...
 */
__device__ void augRieComputeNetUpdates(const real i_hLeft, const real i_hRight, const real i_huLeft,
                                        const real i_huRight, const real i_bLeft, const real i_bRight,

                                        const real g, const real dryTol, const real newtonTol, const real zeroTol,
                                        ...
                                        real o_netUpdates[5])
{
    ...
    else if (hLeft < dryTol)
    {
        ...
        //  => solve homogeneous Riemann-problem to determine the middle state
        //  height
        ...
        computeMiddleState(hRight, hRight, -uRight, uRight, dryTol, newtonTol, g, sqrt_g, maxNumberOfNewtonIterations,
    ...
    else if (hRight < dryTol)
    {
        ...
        //       \cite[ch. 6.8.2]{george2006finite})
        //       \cite[ch. 5.2]{george2008augmented}
        computeMiddleState(hLeft, hLeft, uLeft, -uLeft, dryTol, newtonTol, g, sqrt_g, maxNumberOfNewtonIterations,
                           hMiddle, middleStateSpeeds);
    ...
    if (wetDryState != DryDry)
    {
        ...
        if (wetDryState != WetDryWall and wetDryState != DryWetWall)
        {
            ...
            computeMiddleState(hLeft, hRight, uLeft, uRight, dryTol, newtonTol, g, sqrt_g, 1, hMiddle,
                               middleStateSpeeds);
        }
        ...
        const real hLLMiddleHeight =
            fmax((huLeft - huRight + extEinfeldtSpeeds[1] * hRight - extEinfeldtSpeeds[0] * hLeft) /
...
#pragma message                                                                                                        \
    "strongRarefaction set to false. Further investigations needed about senseless initialization of eigenValues[1]"
        ...
        if (eigenValues[0] < -zeroTol and eigenValues[2] > zeroTol)
        {
            // subsonic
            steadyStateWave[0] =
                fmax(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[0]);
            steadyStateWave[0] =
                fmin(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[2]);
        }
        else if (eigenValues[0] > zeroTol)
        {
            ...
            steadyStateWave[0] =
                fmin(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[0]);
        }
        else if (eigenValues[2] < -zeroTol)
        {
            // supersonic left TODO: motivation?
            steadyStateWave[0] =
                fmax(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[2]);
        ...
        // A^-\delta Q = \sum{s[i]<0} \beta[i] * r[i] = A^-\delta Q = \sum{s[i]<0}
        // Z^i A^+\delta Q = \sum{s[i]>0} \beta[i] * r[i] = A^-\delta Q =
        // \sum{s[i]<0} Z^i
        for (int waveNumber = 0; waveNumber < 3; waveNumber++)
        {
            ...
            {
                // TODO: this case should not happen mathematically, but it does. Where
                // is the bug? Machine accuracy only?
                                          ...
                                          real &o_hMiddle, real o_middleStateSpeeds[2])
{
    ...
    if (riemannStructure == ShockShock)
    {
        ...
        for (unsigned int i = 0; i < i_maxNumberOfNewtonIterations; i++)
        {
            ...
            real phi =
                i_uRight - i_uLeft + (o_hMiddle - i_hLeft) * l_sqrtTermH[0] + (o_hMiddle - i_hRight) * l_sqrtTermH[1];
    ...
    {
        o_hMiddle =
            fmax(static_cast<real>(0), i_uLeft - i_uRight + static_cast<real>(2) * (l_sqrt_g_hLeft + l_sqrt_g_hRight));
