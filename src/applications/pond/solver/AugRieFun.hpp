 ...
 *
 */
template <typename real> class AugRieFun
{
  private:
    const real dryTol;
    ...
    const unsigned int maxNumberOfNewtonIterations; // maximum number of performed
                                                    // Newton iterations

  public:
    /**
     * AugRieFun Constructor, takes three problem parameters
     * @param dryTol "dry tolerance": if the water height falls below dryTol,
     * wall boundary conditions are applied (default value is 100)
     * @param gravity takes the value of the gravity constant (default value
     * is 9.81 m/s^2)
     * @param zeroTol computed f-waves with an absolute value < zeroTol are
     * treated as static waves (default value is 10^{-7})
     */
    AugRieFun(real i_dryTol = (real)100, real i_gravity = (real)9.81, real i_zeroTol = (real)0.0000001,
              real i_newtonTol = (real)0.0000001, real i_maxNewtonIter = 1)
        : dryTol(i_dryTol), g(i_gravity), half_g(static_cast<real>(.5) * i_gravity), sqrt_g(std::sqrt(i_gravity)),
          zeroTol(i_zeroTol), newtonTol(i_newtonTol), maxNumberOfNewtonIterations(i_maxNewtonIter)
    {
    }
                           ...
                           real &o_maxWaveSpeed) const
    {
        ...
        if (hLeft >= dryTol)
        {
            if (hRight >= dryTol)
            {
                // simple wet/wet case - expected as most frequently executed
                // branch
                wetDryState = WetWet;
            }
            else
            { // hLeft >= dryTol and hRight < dryTol
                ...
                { // hLeft >= dryTol and hRight < dryTol and hLeft +
                  // bLeft <= bRight
                    // =>dry cell (right) lies higher than the wet cell (left)
                    // lets check if the momentum is able to overcome the
                    // difference in height
                    //  => solve homogeneous Riemann-problem to determine the
                    //  middle state height
                    //     which would arise if there is a wall
                    //     (wall-boundary-condition)
                    //       \cite[ch. 6.8.2]{george2006finite})
                    //       \cite[ch. 5.2]{george2008augmented}
                    computeMiddleState(hLeft, hLeft, uLeft, -uLeft, maxNumberOfNewtonIterations, hMiddle,
                                       middleStateSpeeds);

                    if (hMiddle + bLeft > bRight)
                    {
                        // momentum is large enough, continue with the original
                        // values
                        //          bRight = o_hMiddle + bLeft;
                        wetDryState = WetDryWallInundation;
                        // limit the effect of the source term if there is a
                        // "wall" \cite[end of ch. 5.2?]{george2008augmented},
        ...
        { // hLeft < dryTol
            if (hRight >= dryTol)
            {
                ...
                { // hLeft < dryTol and hRight >= dryTol and hRight +
                  // bRight <= bLeft
                    ...
                    //       \cite[ch. 6.8.2]{george2006finite})
                    //       \cite[ch. 5.2]{george2008augmented}
                    computeMiddleState(hRight, hRight, -uRight, uRight, maxNumberOfNewtonIterations, hMiddle,
                                       middleStateSpeeds);
                    ...
                    {
                        // momentum is not large enough, use
            ...
            { // hLeft < dryTol and hRight < dryTol
                wetDryState = DryDry;
                // nothing to do for dry/dry case, all netUpdates and
                // maxWaveSpeed are 0
                return;
            }
        };
        ...
        // MB: ???computeMiddleState always called? -> possible to rearrange
        // computation to reduce if-statements??? MB: !!!here always only 1
        // Newton iteration
        ...
        const real hLLMiddleHeight =
            std::max((huLeft - huRight + extEinfeldtSpeeds[1] * hRight - extEinfeldtSpeeds[0] * hLeft) /
                         (extEinfeldtSpeeds[1] - extEinfeldtSpeeds[0]),
                     static_cast<real>(0));

        // define eigenvalues
        const real eigenValues[3] = {extEinfeldtSpeeds[0],
...
#pragma message                                                                                                        \
    "strongRarefaction set to false. Further investigations needed about senseless initialization of eigenValues[1]"
        ...
        real rightHandSide[3] = {hRight - hLeft, huRight - huLeft,
                                 (huRight * uRight + half_g * hRight * hRight) -
                                     (huLeft * uLeft + half_g * hLeft * hLeft)};
        ...
        // (       1           0       1           ) ( beta[0] ) = (
        // rightHandSide[0] ) ( eigenValues[0]    0  eigenValues[2]   ) (
        // beta[1] )   ( rightHandSide[1] ) ( eigenValues[0]^2  1
        // eigenValues[2]^2 ) ( beta[2] )   ( rightHandSide[2] )

        // step 1: solve the following 2x2 system (1st and 3rd column):
        // (       1              1           ) ( beta[0] ) = ( rightHandSide[0]
        // ) ( eigenValues[0]  eigenValues[2]   ) ( beta[2] )   (
        // rightHandSide[1] )
        ...
        beta[1] =
            rightHandSide[2] - eigenValues[0] * eigenValues[0] * beta[0] - eigenValues[2] * eigenValues[2] * beta[2];

#ifdef false
        // compute inverse of 3x3 matrix
        // MB: !!!AVOID COMPUTATION of inverse => see above for better solution
        // for simple matrix MB: TODO -> different choice of matrix in
        // AugRie.hpp -> requires a separate solution procedure
        const real m[3][3] = {{(eigenVectors[1][1] * eigenVectors[2][2] - eigenVectors[1][2] * eigenVectors[2][1]),
        ...
        real fWaves[3][2];  // array to store the three f-wave vectors (2
                            // components each)
        ...
        else
        {
            ...
            for (int waveNumber = 0; waveNumber < 3; waveNumber++)
            {
                ...
                fWaves[waveNumber][1] = beta[waveNumber] * eigenVectors[2][waveNumber]; // 3rd component of the
        ...
        for (int waveNumber = 0; waveNumber < 3; waveNumber++)
        {
            ...
            else
            {
                ...
                // MB: >>> waveSpeeds set to 0 for cases WetDryWall and
                // DryWetWall
    ...
    inline void computeMiddleState(const real &i_hLeft, const real &i_hRight, const real &i_uLeft, const real &i_uRight,
                                   const unsigned int &i_maxNumberOfNewtonIterations, real &o_hMiddle,
                                   real o_middleStateSpeeds[2]) const
    {
        //!!!MB: currently called with
        // i_maxNumberOfNewtonIterations = maxNumberOfNewtonIterations for wall
        // boundaries
        // -> only leads to ShockShock or RarefactionRarefaction -> can be
        // simplified i_maxNumberOfNewtonIterations = 1 (computation of middle
        // states)
        // -> could be changed towards a simplified implementation
        // ==> consider to split method into two or more separate
        // implementations?
        ...
        if (0 <= static_cast<real>(2) * (std::sqrt(g * hMin) - std::sqrt(g * hMax)) + uDif)
        {
            ...
            o_hMiddle = std::max(static_cast<real>(0),
        ...
        o_middleStateSpeeds[0] =
            i_uLeft + static_cast<real>(2) * l_sqrt_g_hLeft - static_cast<real>(3) * sqrt_g_hMiddle;
        o_middleStateSpeeds[1] =
