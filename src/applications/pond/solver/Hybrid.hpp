 ...
 *            Url =
 *{https://github.com/dlgeorge/clawpack-4.x/blob/master/geoclaw/2d/lib/rpn2ez_fast_geo.f}}
 *
 ...
 * Combines the "simple" f-Wave approach with the more complex Augmented Riemann
 * Solver.
 *
...
template <typename T> class solver::Hybrid
{
  ...
  public:
     ...
     */
    Hybrid(T i_dryTolerance = (T)0.01, T i_gravity = (T)9.81, T i_newtonTolerance = (T)0.000001,
           int i_maxNumberOfNewtonIterations = 10, T zeroTolerance = (T)0.000000001)
        : g(i_gravity), dryTol(i_dryTolerance), fWaveSolver(i_dryTolerance, i_gravity, zeroTolerance),
    ...
    void computeNetUpdates(const T &i_hLeft, const T &i_hRight, const T &i_huLeft, const T &i_huRight, const T &i_bLeft,
                           ...
                           T &o_maxWaveSpeed)
    {
        if (useAugmentedRiemannSolver(i_hLeft, i_hRight, i_huLeft, i_huRight, i_bLeft, i_bRight, o_hUpdateLeft,
                                      o_hUpdateRight, o_huUpdateLeft, o_huUpdateRight, o_maxWaveSpeed) == true)
        {

            augRieSolver.computeNetUpdates(i_hLeft, i_hRight, i_huLeft, i_huRight, i_bLeft, i_bRight,

                                           o_hUpdateLeft, o_hUpdateRight, o_huUpdateLeft, o_huUpdateRight,
                                           o_maxWaveSpeed);
     ...
     * Should be used in the CFL-condition.
     *
     * @return A boolean whether the Augmented Riemann Solver should be used or
     * not.
     */
    bool useAugmentedRiemannSolver(const T &i_hLeft, const T &i_hRight, const T &i_huLeft, const T &i_huRight,
                                   ...
                                   T &o_maxWaveSpeed)
    {
        ...
        { // both cells wet?
            ...
            if (std::fabs(i_hLeft - i_hRight) < (T)0.001 * std::min(i_hLeft, i_hRight) && //"small" jump in height
                std::max(uLeft * uLeft, uRight * uRight) / std::max(g * i_hLeft, g * i_hRight) < (T)0.01)
            { //"small" jump in velocity
                ...
                fWaveSolver.computeNetUpdatesHybrid(
                    i_hLeft, i_hRight, i_huLeft, i_huRight, i_bLeft, i_bRight, uLeft, uRight, waveSpeeds,

                    o_hUpdateLeft, o_hUpdateRight, o_huUpdateLeft, o_huUpdateRight, o_maxWaveSpeed);
            }
        }
     ...
     * @param o_counterAugRie will be set to: times the Augmented Riemann solver
     * was used.
     */
