 ...
 *
 * The SIMD_DEFINITIONS.hpp-file needs the control macros set in this file to
 * work properly.
 */
 ...
 *
 * Additionally, declare the macro SHIFT_SIGN_RIGHT to work properly with 32 and
 * 64 bit. Note the INTENTIONALLY FORGOTTEN semicolon at the end of the
 * SHIFT_SIGN_RIGHT-definitions. This forces the user to write the semicolon
 * himself!
 */
#if defined FLOAT64
 ...
 * which is returned by the instruction MOVEMASK, if the instruction is called
 *on a vector with ALL SIGN BITS set Use is as
 *
