 ...
 *    Homepage:
 *http://www5.in.tum.de/wiki/index.php/Dipl.-Math._Alexander_Breuer E-Mail:
 ...
 *              Author = {George, D.L.},
 *              Title = {Finite volume methods and adaptive refinement for
 *tsunami propagation and inundation}, Year = {2006}}
 *
 ...
 *            Pages = {3089--3113},
 *            Publisher = {Elsevier},
 *            Title = {Augmented Riemann solvers for the shallow water equations
 *over variable topography with steady states and inundation}, Volume = {227},
 *            Year = {2008}}
 *
 ...
 *            Url =
 *{https://github.com/clawpack/clawpack-4.x/blob/master/geoclaw/2d/lib}}
 *
 ****
 *
 * Acknowledgments:
...
class solver::AugRie_SIMD : public WavePropagation<real>
{
  ...
  private:
    ...
    //! Newton-tolerance (exit Newton-Raphson-method, if we are close enough to
    //! the root)
    ...
    //! height of our homogeneous Riemann-problem at middle state (computed by
    //! determineMiddleState)
    real hMiddle;
#ifndef VECTOR_NOVEC
    real_vector hMiddle_v;
#endif /* VECTOR_NOVEC */

    //! shock or inner rarefaction speeds of our homogeneous Riemann-problem
    //! (computed by computeMiddleState)
    ...
    //! Riemann-structure of our homogeneous Riemann-problem (determined by
    //! determineRiemannStructure)
  ...
  public:
     ...
     * @param i_newtonTolerance numerical definition of "convergence" (used in
     * the AugRie solver only).
     * @param i_maxNumberOfNewtonIterations maximum steps for the Newton-Raphson
     * method (used in the AugRie solver only).
     * @param i_zeroTolerance numerical definition of zero.
     */
    AugRie_SIMD(real i_dryTolerance = static_cast<real>(0.01), real i_gravity = static_cast<real>(9.81),
                real i_newtonTolerance = static_cast<real>(0.000001), int i_maxNumberOfNewtonIterations = 10,
                real i_zeroTolerance = static_cast<real>(0.00001))
        : WavePropagation<real>(i_dryTolerance, i_gravity, i_zeroTolerance),
#ifndef VECTOR_NOVEC
          DryDry_V(SETV_I(DryDry)), WetWet_V(SETV_I(WetWet)), DryWetInundation_V(SETV_I(DryWetInundation)),
          WetDryInundation_V(SETV_I(WetDryInundation)), DryWetWallInundation_V(SETV_I(DryWetWallInundation)),
          DryWetWall_V(SETV_I(DryWetWall)), WetDryWallInundation_V(SETV_I(WetDryWallInundation)),
          WetDryWall_V(SETV_I(WetDryWall)),
#endif /* VECTOR_NOVEC */
          newtonTol(i_newtonTolerance), maxNumberOfNewtonIterations(i_maxNumberOfNewtonIterations),
#ifndef VECTOR_NOVEC
...
#ifndef VECTOR_NOVEC
          ,
          sqrt_g_v(SETV_R(sqrt_g)), zeroTol_v(SETV_R(zeroTol)), zeroTol_v_neg(SETV_R(-zeroTol)),
          dryTol_v(SETV_R(dryTol)), dryTol_v_neg(SETV_R(-dryTol)), g_v(SETV_R(g))
#endif /* VECTOR_NOVEC */
    {
#ifndef NDEBUG
     ...
     */
    void computeNetUpdates(const real &i_hLeft, const real &i_hRight, const real &i_huLeft, const real &i_huRight,
                           const real &i_bLeft, const real &i_bRight,

                           real &o_hUpdateLeft, real &o_hUpdateRight, real &o_huUpdateLeft, real &o_huUpdateRight,
                           real &o_maxWaveSpeed)
    {
        ...
        // determine the wet/dry state and compute local variables
        // correspondingly
        ...
        if (wetDryState != DryDry)
        {
            ...
            // A^-\delta Q = \sum{s[i]<0} \beta[i] * r[i] = A^-\delta Q =
            // \sum{s[i]<0} Z^i A^+\delta Q = \sum{s[i]>0} \beta[i] * r[i] =
            // A^-\delta Q = \sum{s[i]<0} Z^i
            for (int waveNumber = 0; waveNumber < 3; waveNumber++)
            {
                ...
                {
                    // TODO: this case should not happen mathematically, but it
                    // does. Where is the bug? Machine accuracy only?
...
#ifndef VECTOR_NOVEC
    void computeNetUpdatesSimd(const real *const i_hLeft, const real *const i_hRight, const real *const i_huLeft,
                               const real *const i_huRight, const real *const i_bLeft, const real *const i_bRight,

                               real *const o_hUpdateLeft, real *const o_hUpdateRight, real *const o_huUpdateLeft,
                               real *const o_huUpdateRight, real &o_maxWaveSpeed)
    {
        // store parameters
        ...
        if (MOVEMASK(wetDryMask) != 0)
        {
            ...
            computeWaveDecompositionSimd(fWaves, waveSpeeds, wetDryMask);

            for (int waveNumber = 0; waveNumber < 3; ++waveNumber)
            {
                const real_vector leftMask = ANDV_R(CMP_LT(waveSpeeds[waveNumber], zeroTol_v_neg), wetDryMask);
                const real_vector rightMask =
                    ANDV_R(ANDNOTV_R(leftMask, CMP_GT(waveSpeeds[waveNumber], zeroTol_v)), wetDryMask);
                ...
                assert(MOVEMASK(ORV_R(NOTV_R(wetDryMask), ORV_R(leftMask, ORV_R(rightMask, middleMask)))) ==
                       VECTOR_FULL_MASK);
            }
            ...
            // compute maximum wave speed (-> CFL-condition)
            const real_vector maxEdgeSpeed_v =
                MAXV(MAXV(FABS(ANDV_R(waveSpeeds[0], wetDryMask)), FABS(ANDV_R(waveSpeeds[1], wetDryMask))),
                     FABS(ANDV_R(waveSpeeds[2], wetDryMask)));
            assert(checkVector(maxEdgeSpeed_v));
            // This may be erroneously "optimized out", when compiled with
            // -fstrict-aliasing but there is simply no other possibility to get
            // the single components of a vector
...
#if defined COUNTFLOPS
            flops += 2 * COSTS_SQRTV + 2 * COSTS_MULV + 4 * (COSTS_STOREU + COSTS_BLENDV) + 2 * COSTS_MAXV +
                     3 * COSTS_FABS + VECTOR_LENGTH * COSTS_MAXS;
#endif
        }
        else
        {
  ...
  private:
    ...
    void determineWetDryState()
    {
        ...
        else if (hLeft < dryTol)
        {
            ...
            // lets check if the momentum is able to overcome the difference in
            // height
            //  => solve homogeneous Riemann-problem to determine the middle
            //  state height
            //     which would arise if there is a wall
    ...
    void determineWetDryStateSimd()
    {
...
#if defined COUNTFLOPS
        flops += 8 * COSTS_BLENDV + COSTS_BLENDV_I + 2 * COSTS_CMP_GT + 2 * COSTS_CMP_GE + 2 * COSTS_DIVV +
                 2 * COSTS_ADDV + 4 * COSTS_ZEROV_R + COSTS_ZEROV_I + COSTS_ANDV_R + COSTS_MOVEMASK;
#endif
        ...
        {
            const real_vector dryDryMask =
                ANDNOTV_R(mask_accumulator, ANDV_R(CMP_LT(hLeft_v, dryTol_v), CMP_LT(hRight_v, dryTol_v)));
        ...
        if (MOVEMASK(wallMask_left))
        {
            ...
            computeMiddleStateSimd(hRight_v, hRight_v, uRight_v_neg, uRight_v, wallMask_left,
        ...
        if (MOVEMASK(wallMask_right))
        {
            ...
            computeMiddleStateSimd(hLeft_v, hLeft_v, uLeft_v, uLeft_v_neg, wallMask_right, maxNumberOfNewtonIterations);
     ...
     * @param o_waveSpeeds will be set to: speeds of the linearized waves
     * (eigenvalues).
     */
    inline void computeWaveDecomposition(real o_fWaves[3][2], real o_waveSpeeds[3])
    {
        // compute eigenvalues of the jacobian matrices in states Q_{i-1} and
        // Q_{i} (char. speeds)
        ...
        // compute extended eindfeldt speeds (einfeldt speeds + middle state
        // speeds)
        //  \cite[ch. 5.2]{george2008augmented},
        ...
        // HLL middle state
        //  \cite[theorem 3.1]{george2006finite},
        //  \cite[ch. 4.1]{george2008augmented}
        ...
        //  \cite[ch. 6.7.2]{george2006finite},
        //  \cite[ch. 5.1]{george2008augmented}
        ...
        //  \cite[ch. 4.2.1 \& app. A]{george2008augmented}, \cite[ch. 6.2 \&
        //  ch. 4.4]{george2006finite}
        ...
        //  \cite[ch. 6.5.2]{george2006finite},
        //  \cite[ch. 4.2.3]{george2008augmented}
        if (eigenValues[0] < -zeroTol and eigenValues[2] > zeroTol)
        {
            // subsonic
            steadyStateWave[0] =
                std::max(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[0]);
            steadyStateWave[0] =
                std::min(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[2]);
#if defined COUNTFLOPS
        ...
        else if (eigenValues[0] > zeroTol)
        {
            ...
            steadyStateWave[0] =
                std::min(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[0]);
#if defined COUNTFLOPS
        ...
        else if (eigenValues[2] < -zeroTol)
        {
            ...
            steadyStateWave[0] =
                std::max(steadyStateWave[0], hLLMiddleHeight * (eigenValues[2] - eigenValues[0]) / eigenValues[2]);
        ...
        steadyStateWave[1] =
            std::min(steadyStateWave[1], g * std::max(-hLeft * (bRight - bLeft), -hRight * (bRight - bLeft)));
        steadyStateWave[1] =
            std::max(steadyStateWave[1], g * std::min(-hLeft * (bRight - bLeft), -hRight * (bRight - bLeft)));
        ...
        else
        {
            ...
            for (int waveNumber = 0; waveNumber < 3; waveNumber++)
            {
                ...
                o_fWaves[waveNumber][1] = beta[waveNumber] * eigenVectors[2][waveNumber]; // 3rd component of the
...
#ifndef VECTOR_NOVEC
    inline void computeWaveDecompositionSimd(real_vector o_fWaves[3][2], real_vector o_waveSpeeds[3],
                                             const real_vector wetDryMask)
    {
        static const real_vector half = SETV_R(static_cast<real>(0.5));
        ...
        const real_vector middleState_blendMask =
            ANDV_R(NOTV_R(ORV_R(CAST_INT_TO_REAL_V(CMP_EQ_I(wetDryState_v, WetDryWall_V)),
                                CAST_INT_TO_REAL_V(CMP_EQ_I(wetDryState_v, DryWetWall_V)))),
                   wetDryMask);
        // compute the middle state of the homogeneous Riemann-Problem
        computeMiddleStateSimd(hLeft_v, hRight_v, uLeft_v, uRight_v, middleState_blendMask);
        ...
        const real_vector speeds_t0[2] = {MINV(extEinfeldtSpeeds[0], middleStateSpeeds_v[1]),
                                          MAXV(extEinfeldtSpeeds[1], characteristicSpeeds[1])};
        const real_vector speeds_t1[2] = {MINV(roeSpeeds[0], middleStateSpeeds_v[1]),
                                          MAXV(characteristicSpeeds[1], roeSpeeds[1])};

        const real_vector wetMask = ANDV_R(
            CAST_INT_TO_REAL_V(ORV_I(ORV_I(CMP_EQ_I(wetDryState_v, WetWet_V), CMP_EQ_I(wetDryState_v, WetDryWall_V)),
...
#ifndef NDEBUG
        const real_vector rightDryMask =
            ANDV_R(wetDryMask, ANDNOTV_R(ORV_R(wetMask, leftDryMask), CMP_LT(hRight_v, dryTol_v)));
        assert(MOVEMASK(ANDV_R(rightDryMask, CMP_LE(middleStateSpeeds_v[1], extEinfeldtSpeeds[0]))) == 0);
        assert(MOVEMASK(ORV_R(NOTV_R(wetDryMask), ORV_R(ORV_R(wetMask, leftDryMask), rightDryMask))) ==
               VECTOR_FULL_MASK);
#endif
        ...
        //  \cite[theorem 3.1]{george2006finite},
        //  \cite[ch. 4.1]{george2008augmented}
        const real_vector hLLMiddleHeight =
            MAXV(DIVV(SUBV(ADDV(SUBV(huLeft_v, huRight_v), MULV(extEinfeldtSpeeds[1], hRight_v)),
...
#pragma message                                                                                                        \
    "strongRarefaction set to false; further investigation needed, about senseless initializations of eigenValues[1]"
        ...
        const real_vector subsonic_mask =
            ANDV_R(ANDV_R(CMP_LT(eigenValues[0], zeroTol_v_neg), CMP_GT(eigenValues[2], zeroTol_v)), wetDryMask);
        mask_accumulator = ORV_R(mask_accumulator, subsonic_mask);
        const real_vector supersonic_right_mask =
            ANDV_R(ANDNOTV_R(mask_accumulator, CMP_GT(eigenValues[0], zeroTol_v)), wetDryMask);
        mask_accumulator = ORV_R(mask_accumulator, supersonic_right_mask);
        const real_vector supersonic_left_mask =
            ANDV_R(ANDNOTV_R(mask_accumulator, CMP_LT(eigenValues[2], zeroTol_v_neg)), wetDryMask);
        ...
        const real_vector rightHandSide[3] = {
            SUBV(SUBV(hRight_v, hLeft_v), steadyStateWave[0]), SUBV(huRight_v, huLeft_v),
            SUBV(SUBV(ADDV(MULV(huRight_v, uRight_v), MULV(MULV(MULV(half, g_v), hRight_v), hRight_v)),
        ...
        // everything is ready, solve the equations!
        solveLinearEquationSimd(eigenVectors, rightHandSide, beta);
#if defined COUNTFLOPS
        ...
        // compute f-waves and wave-speeds
        const real_vector DryWetWall_Mask =
            ANDV_R(CAST_INT_TO_REAL_V(CMP_EQ_I(wetDryState_v, DryWetWall_V)), wetDryMask);
        const integer DryWetWall_State = MOVEMASK(DryWetWall_Mask);
        const real_vector WetDryWall_Mask =
            ANDV_R(ANDNOTV_R(DryWetWall_Mask, CAST_INT_TO_REAL_V(CMP_EQ_I(wetDryState_v, WetDryWall_V))), wetDryMask);
        ...
        assert(MOVEMASK(ORV_R(NOTV_R(wetDryMask), ORV_R(DryWetWall_Mask, ORV_R(WetDryWall_Mask, noWall_Mask)))) ==
               VECTOR_FULL_MASK);

#if defined COUNTFLOPS
        flops += 6 * COSTS_ZEROV_R + 10 * COSTS_ADDV + 23 * COSTS_MULV + 2 * COSTS_DIVV + COSTS_SQRTV +
                 11 * COSTS_SUBV + 5 * COSTS_MINV + 6 * COSTS_MAXV + 3 * COSTS_ORV_I + 5 * COSTS_CMP_EQ_I +
                 4 * COSTS_BLENDV + 5 * COSTS_ANDNOTV_R + 3 * COSTS_CMP_LT + 2 * COSTS_CMP_GT + 4 * COSTS_ORV_I +
                 COSTS_ANDV_R + COSTS_NOTV_I + 4 * COSTS_MOVEMASK;
#endif
        ...
        if (noWall_State)
        {
            ...
            for (int waveNumber = 0; waveNumber < 3; ++waveNumber)
            {
                o_fWaves[waveNumber][0] =
                    BLENDV(o_fWaves[waveNumber][0], MULV(beta[waveNumber], eigenVectors[1][waveNumber]), noWall_Mask);
                o_fWaves[waveNumber][1] =
                    BLENDV(o_fWaves[waveNumber][1], MULV(beta[waveNumber], eigenVectors[2][waveNumber]), noWall_Mask);
            }
     ...
     */
    inline void computeMiddleState(const real &i_hLeft, const real &i_hRight, const real &i_uLeft, const real &i_uRight,
                                   const int i_maxNumberOfNewtonIterations = 1)
    {
        ...
        if (riemannStructure == ShockShock)
        {
            ...
            for (int i = 0; i < i_maxNumberOfNewtonIterations; i++)
            {
                ...
                real phi =
                    i_uRight - i_uLeft + (hMiddle - i_hLeft) * l_sqrtTermH[0] + (hMiddle - i_hRight) * l_sqrtTermH[1];

#if defined COUNTFLOPS
                flops +=
                    2 * COSTS_SQRTS + 8 * COSTS_MULS + 4 * COSTS_ADDS + 2 * COSTS_DIVS + 3 * COSTS_SUBS + COSTS_CMPS;
#endif
        ...
        {
            hMiddle = std::max(static_cast<real>(0),
                               i_uLeft - i_uRight + static_cast<real>(2) * (l_sqrt_g_hLeft + l_sqrt_g_hRight));
        ...
        if (riemannStructure == ShockRarefaction or riemannStructure == RarefactionShock)
        {
            ...
            for (int i = 0; i < i_maxNumberOfNewtonIterations; i++)
            {
                ...
                real derivativePhi =
                    sqrtTermHMin - static_cast<real>(0.25) * g * (hMiddle - hMin) / (hMiddle * hMiddle * sqrtTermHMin) +
        ...
        middleStateSpeeds[1] =
            i_uRight - static_cast<real>(2) * l_sqrt_g_hRight + static_cast<real>(3) * sqrt_g_hMiddle;
#if defined COUNTFLOPS
...
#ifndef VECTOR_NOVEC
    inline void computeMiddleStateSimd(const real_vector i_hLeft, const real_vector i_hRight, const real_vector i_uLeft,
                                       const real_vector i_uRight, const real_vector blendMask,
                                       const int i_maxNumberOfNewtonIterations = 1)
    {
        ...
        const real_vector SingleDryMask =
            BLENDV(ZEROV_R(), ANDNOTV_R(DrySingleMask, CMP_LT(i_hRight, dryTol_v)), blendMask);
        ...
        {
            // determine the wave structure of the Riemann-problem
            riemannStructure_v = determineRiemannStructureSimd(i_hLeft, i_hRight, i_uLeft, i_uRight, Riemann_blendMask);
            ...
            if (MOVEMASK(ShockShockMask))
            {
                ...
                for (int i = 0; i < i_maxNumberOfNewtonIterations; ++i)
                {
                    const real_vector l_sqrtTermH[2] = {
                        ...
                        SQRTV(MULV(half_g,
                                   DIVV(ADDV(iteration_variable, i_hRight), MULV(iteration_variable, i_hRight))))};
                    ...
                    const real_vector phi = ADDV(SUBV(i_uRight, i_uLeft), ADDV(MULV(delta_h_left, l_sqrtTermH[0]),
                                                                               MULV(delta_h_right, l_sqrtTermH[1])));
#if defined COUNTFLOPS
                    flops += 2 * COSTS_SQRTV + 3 * COSTS_SUBV + COSTS_DIVV + 2 * COSTS_DIVV + 7 * COSTS_MULV +
                             4 * COSTS_ADDV + COSTS_FABS + COSTS_CMP_GT + COSTS_ANDV_R + COSTS_MOVEMASK;
#endif
                    ...
                    const real_vector quarter_g_div_hMiddle_squared =
                        DIVV(quarter_g, MULV(iteration_variable, iteration_variable));

                    const real_vector derivativePhi =
                        SUBV(ADDV(l_sqrtTermH[0], l_sqrtTermH[1]),
                             MULV(quarter_g_div_hMiddle_squared,
                    ...
                    assert(MOVEMASK(ORV_R(NOTV_R(ShockShockMask), CMP_GE(iteration_variable, dryTol_v))) ==
                           VECTOR_FULL_MASK);
#if defined COUNTFLOPS
            ...
            if (MOVEMASK(RarefactionRarefactionMask))
            {
                ...
                real_vector iteration_variable =
                    MAXV(ZEROV_R(), ADDV(SUBV(i_uLeft, i_uRight), MULV(two, ADDV(l_sqrt_g_hLeft, l_sqrt_g_hRight))));
...
#if defined COUNTFLOPS
                flops += COSTS_MAXV + COSTS_ZEROV_R + 2 * COSTS_ADDV + COSTS_SUBV + 4 * COSTS_MULV + COSTS_DIVV +
                         COSTS_BLENDV;
#endif
            }

            sqrt_g_hMiddle = BLENDV(sqrt_g_hMiddle, SQRTV(MULV(g_v, l_hMiddle)),
                                    BLENDV(ZEROV_R(), ORV_R(ShockShockMask, RarefactionRarefactionMask), blendMask));

            const real_vector ShockRarefactionMask =
                BLENDV(ZEROV_R(), CAST_INT_TO_REAL_V(SHIFT_LEFT(riemannStructure_v, 2)), blendMask);
            const real_vector RarefactionShockMask =
                BLENDV(ZEROV_R(), CAST_INT_TO_REAL_V(SHIFT_LEFT(riemannStructure_v, 3)), blendMask);
            ...
            if (MOVEMASK(MixShockRarefactionMask))
            {
                ...
                for (int i = 0; i < i_maxNumberOfNewtonIterations; ++i)
                {
                    const real_vector sqrtTermHMin =
                        SQRTV(MULV(half_g, DIVV(ADDV(iteration_variable, hMin), MULV(iteration_variable, hMin))));

                    const real_vector delta_h_min = SUBV(iteration_variable, hMin);

                    const real_vector phi =
                        ADDV(SUBV(i_uRight, i_uLeft), ADDV(MULV(delta_h_min, sqrtTermHMin),
                                                           MULV(two, SUBV(iteration_variable_sqrt, sqrt_g_hMax))));
                    ...
                    const real_vector derivativePhi = ADDV(
                        SUBV(sqrtTermHMin,
                             MULV(quarter_g,
                                  DIVV(delta_h_min, MULV(MULV(iteration_variable, iteration_variable), sqrtTermHMin)))),
...
#if defined COUNTFLOPS
            flops += COSTS_BLENDV + COSTS_NOTV_R + COSTS_ZEROV_R + COSTS_SQRTV + 4 * COSTS_MULV + COSTS_ADDV +
                     3 * COSTS_SUBV + 3 * COSTS_SHIFT_LEFT + COSTS_ANDV_R + COSTS_ORV_R + 3 * COSTS_MOVEMASK;
#endif
            assert(MOVEMASK(ORV_R(NOTV_R(blendMask),
                                  ORV_R(DrySingleMask, ORV_R(SingleDryMask, ORV_R(ShockShockMask,
                                                                                  ORV_R(RarefactionRarefactionMask,
                                                                                        MixShockRarefactionMask)))))) ==
                   VECTOR_FULL_MASK);
        }
...
#if defined COUNTFLOPS
        flops += COSTS_SETV_R + 3 * COSTS_ZEROV_R + 2 * COSTS_SQRTV + 4 * COSTS_MULV + 2 * COSTS_CMP_LT +
                 COSTS_ANDNOTV_R + COSTS_ORV_R + COSTS_NOTV_R + COSTS_MOVEMASK + COSTS_ADDV + COSTS_SUBV +
                 2 * COSTS_BLENDV_I + 7 * COSTS_BLENDV;
#endif
    }
#endif /* VECTOR_NOVEC */
     ...
     *   -> \cite[theorem 4.2]{george2006finite}, \cite[appendix
     * B]{george2008augmented}
     *
     * @param i_hLeft height on the left side of the edge.
     * @param i_hRight height on the right side of the edge.
     * @param i_uLeft velocity on the left side of the edge.
     ...
     */
    inline integer determineRiemannStructure(const real &i_hLeft, const real &i_hRight, const real &i_uLeft,
                                             const real &i_uRight) const
    {
...
#ifndef VECTOR_NOVEC
    inline integer_vector determineRiemannStructureSimd(const real_vector i_hLeft, const real_vector i_hRight,
                                                        const real_vector i_uLeft, const real_vector i_uRight,
                                                        const real_vector blendMask) const
    {
        static const real_vector two = SETV_R(static_cast<real>(2));
        ...
        const real_vector RarefactionRarefactionMask =
            BLENDV(ZEROV_R(), CMP_GE(condition_value_first, uDif), blendMask);
        ...
        if (MOVEMASK(ORV_R(NOTV_R(blendMask), RarefactionRarefactionMask)) != VECTOR_FULL_MASK)
        {
            ...
            static const real_vector one = SETV_R(static_cast<real>(1));
            // Note that the following line will cause errors, if the
            // gravitational constant g changes during the simulation
            static const real_vector half_g = MULV(g_v, SETV_R(static_cast<real>(0.5)));

            const real_vector condition_value_second =
                MULV(SUBV(hMax, hMin), SQRTV(MULV(half_g, ADDV(DIVV(one, hMax), DIVV(one, hMin)))));
            const real_vector ShockShockMask =
                BLENDV(ZEROV_R(), ANDNOTV_R(mask_accumulator, CMP_LE(condition_value_second, uDif)), blendMask);
            ...
            const real_vector ShockRarefactionMask =
                BLENDV(ZEROV_R(), ANDNOTV_R(mask_accumulator, CMP_LT(i_hLeft, i_hRight)), blendMask);
    ...
    static inline void solveLinearEquation(const real i_matrix[3][3], const real i_b[3], real o_x[3])
    {
        ...
        const real m[3][3] = {{i_matrix[1][1] * i_matrix[2][2] - i_matrix[1][2] * i_matrix[2][1],
        ...
        // m stores not really the inverse matrix, but the inverse multiplied by
        // d
        real s = 1 / d;
...
#ifndef VECTOR_NOVEC
    inline void solveLinearEquationSimd(const real_vector i_matrix[3][3], const real_vector i_b[3],
                                        real_vector o_x[3]) const
    {
        // Vectorization works, but results are different from file 5 up to the
        // end
...
/*
 * The results of the following two versions of the same (!) statement, differ
 * up to 20% in the momentum-updates
 */
#if 0
		/*
...
#else
        /*
         * this version is tested, even though nobody knows whether it is the
         * right one
         */
        const real_vector d =
            ADDV(MULV(i_matrix[0][0], m[0][0]), ADDV(MULV(i_matrix[0][1], m[1][0]), MULV(i_matrix[0][2], m[2][0])));
#endif
