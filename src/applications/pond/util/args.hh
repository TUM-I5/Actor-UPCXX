...
class Args
{
  ...
  public:
    Args(const std::string &description = "", bool addHelp = true) : m_description(description), m_addHelp(addHelp) {}
    ...
    void addOption(const std::string &longOption, char shortOption = 0, const std::string &description = "", Argument argument = Required, bool required = true)
    {
        ...
        if (shortOption)
            m_short2option[(size_t)shortOption] = m_options.size();
        ...
        struct option o = {m_optionInfo.back().longOption.c_str(), argument, nullptr, shortOption};
    ...
    Result parse(int argc, char **argv, bool printHelp = true)
    {
        ...
        for (std::vector<struct option>::const_iterator i = m_options.begin(); i != m_options.end(); i++)
        {
            if (i->val != 0)
            {
        ...
        for (std::vector<optionInfo>::const_iterator i = m_optionInfo.begin(); i != m_optionInfo.end(); i++)
        {
    ...
    bool isSet(const std::string &option) { return m_arguments.find(option) != m_arguments.end(); }
    ...
    template <typename T> T getArgument(const std::string &option, T defaultArgument)
    {
        if (!isSet(option))
...
template <> inline std::string tools::Args::getArgument(const std::string &option) { return m_arguments.at(option); }
