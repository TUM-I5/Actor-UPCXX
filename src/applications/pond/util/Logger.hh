...
class tools::Logger {
   ...
   */
  void printNumber1d(const int i_nX, const std::string i_quantity) {
   ...
   * @param i_quantity definition of the qantity.
   */
  void printNumber2d(const int i_nX, const int i_nY,
...
public:
   ...
   * @param i_indentation definition of the indentation (used in all messages,
   * except welcome, start and finish).
   */
  Logger(
      const int i_processRank = 0, const std::string i_programName = "Pond",
      ...
      : processRank(i_processRank), programName(i_programName),
        welcomeMessage(i_welcomeMessage), copyRights(i_copyRights),
        finishMessage(i_finishMessage), midDelimiter(i_midDelimiter),
        largeDelimiter(i_largeDelimiter), indentation(i_indentation) {
  ...
  void printWelcomeMessage() {
    if (processRank == 0) {
      std::cout << largeDelimiter << welcomeMessage << " " << programName
                << copyRights << largeDelimiter;
    }
  }
  ...
  void printFinishMessage() {
    if (processRank == 0) {
      std::cout << largeDelimiter << programName << " " << finishMessage
                << largeDelimiter;
    }
  }
   ...
   */
  void setProcessRank(const int i_processRank) { processRank = i_processRank; }
   ...
   */
  void
  printNumberOfProcesses(const int i_numberOfProcesses,
                         const std::string i_processesName = "MPI processes") {
    if (processRank == 0)
      timeCout() << indentation << "Number of " << i_processesName << ": "
   ...
   */
  void printNumberOfCells(const int i_nX, const int i_nY,
   ...
   */
  void printCellSize(const float i_dX, const float i_dY,
                     const std::string i_unit = "m") {
    if (processRank == 0) {
      timeCout() << indentation << "Cell size: " << i_dX << i_unit << " * "
                 << i_dY << i_unit << " = " << i_dX * i_dY << " " << i_unit
                 << "^2" << std::endl;
    }
  }
   ...
   * (process rank 0 only)
   */
  void printStartMessage(const std::string i_startMessage =
  ...
  void printSimulationTime(
    ...
    if (processRank == 0) {
      timeCout() << indentation << i_simulationTimeMessage << ": " << i_time
                 << " seconds." << std::endl;
    }
  }
   ...
   */
  void printOutputFileCreation(const std::string i_fileName, const int i_blockX,
                               const int i_blockY,
                               const std::string i_fileType = "netCDF") {
    timeCout() << indentation << "process " << processRank << " - "
               << "creating " << i_fileType << " file " << i_fileName
               << " for block " << i_blockX << ", " << i_blockY << "."
               << std::endl;
  }
  ...
  void printOutputTime(
    ...
    if (processRank == 0) {
      timeCout() << indentation << i_outputTimeMessage << ": " << i_time
                 << " seconds" << std::endl;
    }
  }
  ...
  void printStatisticsMessage(
      const std::string i_statisticsMessage =
  ...
  void printSolverStatistics(
      const long i_firstSolverCounter, const long i_secondSolverCounter,
      const int i_blockX = 0, const int i_blockY = 0,
      const std::string i_firstSolverName = "f-Wave solver",
      const std::string i_secondSolverName = "Augemented Riemann solver") {
    timeCout() << indentation << "process " << processRank << " - "
               << "Solver Statistics for block " << i_blockX << ", " << i_blockY
               << ":" << std::endl;
    timeCout() << indentation << "process " << processRank << " - "
               << indentation << "Times the " << i_firstSolverName
               << " was used: " << i_firstSolverCounter << std::endl;
    timeCout() << indentation << "process " << processRank << " - "
               << indentation << "Times the " << i_secondSolverName
               << " was used: " << i_secondSolverCounter << std::endl;
    timeCout() << indentation << "process " << processRank << " - "
               << indentation
               << "In Total: " << i_firstSolverCounter + i_secondSolverCounter
               << std::endl;
  }
   ...
   */
  void printWallClockTime(
      const double i_wallClockTime,
      const std::string i_wallClockTimeMessage = "wall clock time") {
    timeCout() << indentation << "process " << processRank << " - "
               << i_wallClockTimeMessage << ": "
               << i_wallClockTime - wallClockTime << " seconds" << std::endl;
  }
  ...
  void printTime(const std::string &i_name, const std::string &i_message) {
    timeCout() << indentation << "process " << processRank << " - " << i_message
   ...
   * @return elapsed time
   */
  double getTime(const std::string &i_name) { return timer.at(i_name); }
   ...
   */
  void printIterationsDone(unsigned int i_iterations,
                           std::string i_iterationMessage = "iterations done") {
    if (processRank == 0) {
      timeCout() << indentation << i_iterations << ' ' << i_iterationMessage
                 << std::endl;
    }
  }
   ...
   * @param i_interationMessage Iterations done message
   */
  void printElementUpdatesDone(unsigned int i_iterations, const int i_nX,
                               const int i_nY, const std::string &i_name,
                               const std::string i_iterationMessage =
                                   "element updates per second done") {
    if (processRank == 0) {
      timeCout() << indentation
                 << double(i_iterations) * i_nX * i_nY / timer.at(i_name) << ' '
