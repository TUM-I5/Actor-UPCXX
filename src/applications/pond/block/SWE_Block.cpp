...
SWE_Block::SWE_Block(int l_nx, int l_ny, float l_dx, float l_dy)
    : nx(l_nx), ny(l_ny), dx(l_dx), dy(l_dy), h(nx + 2, ny + 2), hu(nx + 2, ny + 2), hv(nx + 2, ny + 2),
      b(nx + 2, ny + 2),
      // r(nx + 2, ny + 2),
      ...
      maxTimestep(0), offsetX(0), offsetY(0)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        boundary[i] = BoundaryType::PASSIVE;
        neighbour[i] = nullptr;
    };
...
SWE_Block::SWE_Block(SWEBlockData &&sbd)
    : nx(sbd.nx), ny(sbd.ny), dx(sbd.dx), dy(sbd.dy), h(std::move(sbd.h)), hu(std::move(sbd.hu)), hv(std::move(sbd.hv)),
      b(std::move(sbd.b)),
      //  r(nx + 2, ny + 2),
      maxTimestep(sbd.maxTimeStep), offsetX(sbd.offsetX), offsetY(sbd.offsetY)
{
    for (int i = 0; i < 4; i++)
    {
        boundary[i] = sbd.boundary[i];
        // neighbour[i] = &(sbd.neighbour[i]);
        neighbour[i] = nullptr;
    };
    /*int rank = upcxx::rank_me();
    for (int i = 1; i <= nx; i++) {
...
SWE_Block::SWE_Block(SWE_Block &&other)
    : nx(other.nx), ny(other.ny), dx(other.dx), dy(other.dy), h(std::move(other.h)), hu(std::move(other.hu)),
      hv(std::move(other.hv)), b(std::move(other.b)),
      //  r(nx + 2, ny + 2),
      maxTimestep(other.maxTimestep), offsetX(other.offsetX), offsetY(other.offsetY)
{
}

SWE_Block::SWE_Block(const SWE_Block &other)
    : nx(other.nx), ny(other.ny), dx(other.dx), dy(other.dy), h(other.h), hu(other.hu), hv(other.hv), b(other.b),
      //  r(nx + 2, ny + 2),
      maxTimestep(other.maxTimestep), offsetX(other.offsetX), offsetY(other.offsetY)
{
}
...
/**
 * Initializes the unknowns and bathymetry in all grid cells according to the
 * given SWE_Scenario.
 *
 * In the case of multiple SWE_Blocks at this point, it is not clear how the
 * boundary conditions should be set. This is because an isolated SWE_Block
 * doesn't have any in information about the grid. Therefore the calling
 * routine, which has the information about multiple blocks, has to take care
 * about setting the right boundary conditions.
 *
 * @param i_scenario scenario, which is used during the setup.
 * @param i_multipleBlocks are the multiple SWE_blocks?
 */
void SWE_Block::initScenario(float _offsetX, float _offsetY, const Scenario &i_scenario, const bool i_multipleBlocks)
{
    offsetX = _offsetX;
    offsetY = _offsetY;
    ...
    if (i_multipleBlocks == false)
    {
        ...
        setBoundaryType(BoundaryEdge::BND_LEFT, i_scenario.getBoundaryType(BoundaryEdge::BND_LEFT));
        setBoundaryType(BoundaryEdge::BND_RIGHT, i_scenario.getBoundaryType(BoundaryEdge::BND_RIGHT));
        setBoundaryType(BoundaryEdge::BND_BOTTOM, i_scenario.getBoundaryType(BoundaryEdge::BND_BOTTOM));
        setBoundaryType(BoundaryEdge::BND_TOP, i_scenario.getBoundaryType(BoundaryEdge::BND_TOP));
    }
 ...
 * Note: unknowns hu and hv represent momentum, while parameters u and v are
 * velocities!
 */
 ...
 */
const Float2D &SWE_Block::getDischargeHu()
{
    synchDischargeBeforeRead();
    return hu;
}

/*const Float2D &SWE_Block::getRank() {
    synchDischargeBeforeRead();
    return r;
}*/
 ...
 */
const Float2D &SWE_Block::getDischargeHv()
{
    synchDischargeBeforeRead();
    return hv;
}
 ...
 */
void SWE_Block::simulateTimestep(float dt)
{
 ...
 * Note: this implementation can only be used, if you only use a single
 * SWE_Block and only apply simple boundary conditions! In particular,
 * SWE_Block::simulate can not trigger calls to exchange values of copy and
 * ghost layers between blocks!
 * @param	tStart	time where the simulation is started
 * @param	tEnd	time of the next checkpoint
 * @return	actual	end time reached
 */
float SWE_Block::simulate(float i_tStart, float i_tEnd)
{
    float t = i_tStart;
    do
    {
 ...
 * @param i_inflow pointer to an SWE_Block1D, which specifies the inflow (should
 * be NULL for WALL or OUTFLOW boundary)
 */
void SWE_Block::setBoundaryType(const BoundaryEdge i_edge, const BoundaryType i_boundaryType,
                                const SWE_Block1D *i_inflow)
{
    boundary[(int)i_edge] = i_boundaryType;
    neighbour[(int)i_edge] = i_inflow;

    if (i_boundaryType == BoundaryType::OUTFLOW || i_boundaryType == BoundaryType::WALL)
...
void SWE_Block::setBoundaryBathymetry()
{
    ...
    if (boundary[(int)BoundaryEdge::BND_LEFT] == BoundaryType::OUTFLOW ||
        boundary[(int)BoundaryEdge::BND_LEFT] == BoundaryType::WALL)
    {
        memcpy(b[0], b[1], sizeof(float) * (ny + 2));
    }
    if (boundary[(int)BoundaryEdge::BND_RIGHT] == BoundaryType::OUTFLOW ||
        boundary[(int)BoundaryEdge::BND_RIGHT] == BoundaryType::WALL)
    {
        memcpy(b[nx + 1], b[nx], sizeof(float) * (ny + 2));
    }
    if (boundary[(int)BoundaryEdge::BND_BOTTOM] == BoundaryType::OUTFLOW ||
        boundary[(int)BoundaryEdge::BND_BOTTOM] == BoundaryType::WALL)
    {
        for (int i = 0; i <= nx + 1; i++)
        {
            b[i][0] = b[i][1];
    ...
    if (boundary[(int)BoundaryEdge::BND_TOP] == BoundaryType::OUTFLOW ||
        boundary[(int)BoundaryEdge::BND_TOP] == BoundaryType::WALL)
    {
        for (int i = 0; i <= nx + 1; i++)
        {
            b[i][ny + 1] = b[i][ny];
 ...
 */
std::unique_ptr<SWE_Block1D> SWE_Block::registerCopyLayer(BoundaryEdge edge)
{

    switch (edge)
    {
    case BoundaryEdge::BND_LEFT:
        return std::unique_ptr<SWE_Block1D>(::new SWE_Block1D(h.getColProxy(1), hu.getColProxy(1), hv.getColProxy(1)));
    case BoundaryEdge::BND_RIGHT:
        return std::unique_ptr<SWE_Block1D>(
            ::new SWE_Block1D(h.getColProxy(nx), hu.getColProxy(nx), hv.getColProxy(nx)));
    case BoundaryEdge::BND_BOTTOM:
        return std::unique_ptr<SWE_Block1D>(::new SWE_Block1D(h.getRowProxy(1), hu.getRowProxy(1), hv.getRowProxy(1)));
    case BoundaryEdge::BND_TOP:
        return std::unique_ptr<SWE_Block1D>(
            ::new SWE_Block1D(h.getRowProxy(ny), hu.getRowProxy(ny), hv.getRowProxy(ny)));
    };
    return nullptr;
}

/**
 * "grab" the ghost layer at the specific boundary in order to set boundary
 * values in this ghost layer externally. The boundary conditions at the
 * respective ghost layer is set to PASSIVE, such that the grabbing program
 * component is responsible to provide correct values in the ghost layer, for
 * example by receiving data from a remote copy layer via MPI communication.
 * @param	specified edge
 * @return	a SWE_Block1D object that contains row variables h, hu, and hv
 */
std::unique_ptr<SWE_Block1D> SWE_Block::grabGhostLayer(BoundaryEdge edge)
{

    boundary[(int)edge] = BoundaryType::PASSIVE;
    switch (edge)
    {
    case BoundaryEdge::BND_LEFT:
        return std::unique_ptr<SWE_Block1D>(new SWE_Block1D(h.getColProxy(0), hu.getColProxy(0), hv.getColProxy(0)));
    case BoundaryEdge::BND_RIGHT:
        return std::unique_ptr<SWE_Block1D>(
            new SWE_Block1D(h.getColProxy(nx + 1), hu.getColProxy(nx + 1), hv.getColProxy(nx + 1)));
    case BoundaryEdge::BND_BOTTOM:
        return std::unique_ptr<SWE_Block1D>(new SWE_Block1D(h.getRowProxy(0), hu.getRowProxy(0), hv.getRowProxy(0)));
    case BoundaryEdge::BND_TOP:
        return std::unique_ptr<SWE_Block1D>(
            new SWE_Block1D(h.getRowProxy(ny + 1), hu.getRowProxy(ny + 1), hv.getRowProxy(ny + 1)));
    };
    return nullptr;
}
...
void SWE_Block::setGhostLayer()
{
    ...
    // -> these copy operations cannot be executed in GPU/accelerator memory,
    // e.g.
...
#endif
    // left boundary
    if (boundary[(int)BoundaryEdge::BND_LEFT] == BoundaryType::CONNECT)
    {
        for (int j = 0; j <= ny + 1; j++)
        {
            h[0][j] = neighbour[(int)BoundaryEdge::BND_LEFT]->h[j];
            hu[0][j] = neighbour[(int)BoundaryEdge::BND_LEFT]->hu[j];
            hv[0][j] = neighbour[(int)BoundaryEdge::BND_LEFT]->hv[j];
        };
    };

    // right boundary
    if (boundary[(int)BoundaryEdge::BND_RIGHT] == BoundaryType::CONNECT)
    {
        for (int j = 0; j <= ny + 1; j++)
        {
            h[nx + 1][j] = neighbour[(int)BoundaryEdge::BND_RIGHT]->h[j];
            hu[nx + 1][j] = neighbour[(int)BoundaryEdge::BND_RIGHT]->hu[j];
            hv[nx + 1][j] = neighbour[(int)BoundaryEdge::BND_RIGHT]->hv[j];
        };
    };

    // bottom boundary
    if (boundary[(int)BoundaryEdge::BND_BOTTOM] == BoundaryType::CONNECT)
    {
        for (int i = 0; i <= nx + 1; i++)
        {
            h[i][0] = neighbour[(int)BoundaryEdge::BND_BOTTOM]->h[i];
            hu[i][0] = neighbour[(int)BoundaryEdge::BND_BOTTOM]->hu[i];
            hv[i][0] = neighbour[(int)BoundaryEdge::BND_BOTTOM]->hv[i];
        };
    };

    // top boundary
    if (boundary[(int)BoundaryEdge::BND_TOP] == BoundaryType::CONNECT)
    {
        for (int i = 0; i <= nx + 1; i++)
        {
            h[i][ny + 1] = neighbour[(int)BoundaryEdge::BND_TOP]->h[i];
            hu[i][ny + 1] = neighbour[(int)BoundaryEdge::BND_TOP]->hu[i];
            hv[i][ny + 1] = neighbour[(int)BoundaryEdge::BND_TOP]->hv[i];
        }
    };
 ...
 */
void SWE_Block::computeMaxTimestep(const float i_dryTol, const float i_cflNumber)
{
    ...
    for (int i = 1; i <= nx; i++)
    {
        for (int j = 1; j <= ny; j++)
        {
            if (h[i][j] > i_dryTol)
            {
                double l_momentum = std::max(std::abs(hu[i][j]), std::abs(hv[i][j]));
...
void SWE_Block::setBoundaryConditions()
{
    ...
    // PASSIVE boundary conditions need to be set by the component using
    // SWE_Block

    // left boundary
    switch (boundary[(int)BoundaryEdge::BND_LEFT])
    {
    ...
    // right boundary
    switch (boundary[(int)BoundaryEdge::BND_RIGHT])
    {
    case BoundaryType::WALL:
    {
        for (int j = 1; j <= ny; j++)
    ...
    // bottom boundary
    switch (boundary[(int)BoundaryEdge::BND_BOTTOM])
    {
    case BoundaryType::WALL:
    {
        for (int i = 1; i <= nx; i++)
    ...
    // top boundary
    switch (boundary[(int)BoundaryEdge::BND_TOP])
    {
    case BoundaryType::WALL:
    {
        for (int i = 1; i <= nx; i++)
    ...
    /*
     * Set values in corner ghost cells. Required for dimensional splitting and
     * visualizuation. The quantities in the corner ghost cells are chosen to
     * generate a zero Riemann solutions (steady state) with the neighboring
     * cells. For the lower left corner (0,0) using the values of (1,1)
     * generates a steady state (zero) Riemann problem for (0,0) - (0,1) and
     ...
     *                  *  _    _    *  _    _   *
     *  Ghost           * |  h   |   * |  h   |  *
     *  cell    ------> * | -hu  |   * |  hu  |  * <------ Cell (1,1) inside the
     * domain (0,1)           * |_ hv _|   * |_ hv _|  *
     *                  *            *           *
     *                  **************************
     *                  *  _    _    *  _    _   *
     *   Corner Ghost   * |  h   |   * |  h   |  *
...
bool SWE_Block::displacement(float defVal)
{
    for (int i = 1; i <= nx; i++)
    {
        for (int j = 1; j <= ny; j++)
        {
            float diff = defVal - h[i][j];
            if (diff > 0.1 || diff < -0.1)
            {
                // std::cout << defVal << " - " << h[i][j] << std::endl;
                return true;
            }
        }
    }
